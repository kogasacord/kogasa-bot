...
export async function execute(
	...
	if (query && query.length <= 0) {
		msg.reply("Please consult `??help remindme` for more information regarding this command.");
		return;
	}
...
function userRemoveReminder(msg: Message, reminder_emitter: ReminderEmitter, args: string[]) {
	...
	if (reminder) {
		msg.reply(`Removed the reminder scheduled at ${reminder.to_date.toUTCString()}.`);
	} else {
		msg.reply("Unable to remove the reminder you selected.");
	}

}
function listReminders(reminder_emitter: ReminderEmitter, user_id: string): EmbedBuilder {
	...
	for (let i = 0; i < reminder_contents.length; i++) {
		...
		embed_fields.push({ name: `${i + 1}) ${r.to_date.toUTCString()}`, value: r.contents });
	}
	embed.addFields(embed_fields);
	return embed;
}

function userAddReminder(msg: Message, reminder_emitter: ReminderEmitter, args: string[]) {
	...
	if (reminders && reminders.length >= 20) {
		msg.reply("You have too many reminders! Remove excess reminders using `??remindme remove`");
	}
