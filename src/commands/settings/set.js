...
export async function execute(client, msg, args, ext) {
    ...
    const server_setting = await findThroughCollection(settings, "serverid", msg.channel.guildId);
    ...
    if (channel_id === "all") {
        for (const command of commands) {
            for (const channel of guild.channels.cache) {
                await createCommandScope(command_scopes, channel_ids, settings, command.name, channel[1].id, isEnabled, server_setting.id);
            }
        }
        ...
        let message = "";
        for (const command of commands) {
            const scope = await createCommandScope(command_scopes, channel_ids, settings, command.name, channel_id, isEnabled, server_setting.id);
            message = `${isEnabled ? "Activated" : "Disabled"} \`${formatScope(scope)}\` into this channel.`;
        }
        msg.reply(message);
    }
}
async function createCommandScope(command_scopes, channel_ids, settings, command_name, channel_id, isEnabled, server_setting_id) {
    const channel = await findThroughCollection(channel_ids, "channel_id", channel_id);
    if (channel) {
        const channel_record = await channel_ids.getOne(channel.id, { expand: "command_scope" });
        ...
        const scope = await command_scopes.create({
            [command_name]: isEnabled
        });
        const channel_id_record = await channel_ids.create({
...
async function initializeSettings(settings, guildID) {
    const default_settings = {
        serverid: guildID,
        nsfw: false,
        self_quote: true,
    };
    return await settings.create(default_settings);
}
...
async function checkMatchingServerChannelIDs(client, channel_id, server_id) {
    const guild = client.guilds.cache.get(server_id) ?? await client.guilds.fetch(server_id);
