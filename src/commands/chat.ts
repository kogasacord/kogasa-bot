...
const MAX_LENGTH = 10
const SYS_PROMPT =
  "Reply as Youmu Konpaku, not as an A.I." +
...
export async function execute(
  ...
  const user = await helpers.findThroughCollection<PBUsers>(
    users,
    "user_id",
    msg.author.id
  )
  ...
  const message_id_pb_queued_buffer = await queue_simulate(
    messages,
    message_id_pb_buffer,
    MAX_LENGTH
  )
  // updates the user's messages
  await users.update(
    user!.id,
    { messages: message_id_pb_queued_buffer },
    { $autoCancel: false }
  )
...
async function queue_simulate(
  messages: RecordService,
  array: string[],
  max_length: number
) {
