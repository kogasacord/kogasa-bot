...
export async function messageDelete(client: Client, partial_new_msg: Message<boolean> | PartialMessage, chat_buffer: ChatBuffer) {
	const chat_buffer_queue = setChatbuffer(chat_buffer, partial_new_msg.channelId);
	...
	const chat_buffer_message = findChatBufferMessagewithRealMessage(chat_buffer_queue, partial_new_msg);
