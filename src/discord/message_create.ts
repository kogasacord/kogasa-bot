...
import {CommandModule, Cooldown, ExternalDependencies, Website, Tier, DiscordExternalDependencies} from "../helpers/types.js";
...
export async function messageCreate(
	...
	const prefix = await helpers.prefixChange(pb, deps.settings.test, msg.channel.guildId);
	...
	const command_module = aliasToCommand(deps.aliases, deps.commands, alias_command_name);
  ...
  try {
    ...
    if (!command_module.noscope) {
				if (await helpers.commandChannelAccess(pb, command_module.name, msg.channel.id, msg.guild!.id)) {
    ...
    const ext: ExternalDependencies = {
      ...
      prefix: prefix,
      external_data: [websites, tiers, deps.chat_buffer],
    }
