...
function hasAuthorCooldownPassed(
	author_timestamp: Cooldown,
	now: number,
) {
...
async function executeCommandChecker(command_module: CommandModule, msg: Message, args: string[]) {
...
async function getDynamicCooldown(args: string[], dyn_cooldown?: (args: string[]) => Promise<number>): Promise<number> {
	return dyn_cooldown
		? (await dyn_cooldown(args)) * 1000
		: 0
}
...
async function setAuthorCooldown(
	...
	const cooldownAdditional = await getDynamicCooldown(args, command_module.dyn_cooldown);
...
export async function implementCooldown(
	...
	checkCooldownHasPassed(author_timestamps, msg, command_module.name, author_id, now);
