...
export async function commandChannelAccess(
    ...
    if (!server_record && (command_name !== "set")) {
        return `Please \`${prefix}set\` up the server first. Use this command: \`${prefix}set [command_name] [channel_id] [true/false]\` to set up channels.`;
    }

    if (!channel_record && (command_name !== "set")) {
